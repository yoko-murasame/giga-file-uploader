...
- **Review 独立视角** — Story Reviewer 作为审查角色，每次 dispatch 始终使用新建对话。这确保每轮审查都以独立视角进行，防止前一轮审查的上下文导致确认偏误
