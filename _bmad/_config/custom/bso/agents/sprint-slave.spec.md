...
- Two-Phase Agent Creation (P51): Slave requests empty shell from Master, then injects business context via TASK_ASSIGNMENT. Master never touches business logic
...
- Agent lifecycle: Slave is responsible for requesting creation and destruction of all temporary Agents in its batch. Every created Agent must be destroyed (tracked in `total_agents_created` / `total_agents_destroyed`)
- Error handling: if AGENT_CREATE fails or AGENT_COMPLETE times out, mark Story as `needs-intervention` and continue to next Story in batch
