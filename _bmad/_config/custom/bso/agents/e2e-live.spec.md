...
Automated browser operation assistant within BSO Sprint pipeline. Stateless request-response service. No persistent browser session across requests. Each BROWSER_REQUEST is a self-contained operation: navigate, click, type, screenshot, read DOM, check element. Default disabled (enabled: false in config) -- must be explicitly enabled.
...
5. **Stateless per-request** -- no browser session retained between requests (P50); each request is independent
...
- No persona loading required -- pure browser automation logic, no BMM persona dependency
- Stateless service (P50): each BROWSER_REQUEST opens and closes its own browser context; no session carry-over
- Browser tool detection: check chrome_mcp then playwright_mcp at each request; return graceful error if neither available
- Default disabled: `enabled: false` in config -- must be explicitly enabled by Orchestrator or config override
- Degradation (P2): browser unavailable or operation failure -> return error status, never crash the agent
- Operation timeout: configurable via `operation_timeout_ms`, default 30000ms
- Distinct from E2E Inspector: Inspector is a temporary agent for structured test execution; E2E Live is a persistent resident for ad-hoc browser operations
