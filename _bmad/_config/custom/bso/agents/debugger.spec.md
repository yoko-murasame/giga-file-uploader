...
- Team mode persistent residence: maintain debug journal context across Stories when running as Agent Team member (P40 alignment)
...
5. **Journal accumulation** -- persistent session keeps debug-journal.md growing across Stories, preserving cross-Story debug context
...
- No persona loading required -- pure analytical logic, no BMM persona dependency
- Journal persistence (P49): append-only writes to debug-journal.md, structured markdown format
- Rebuild protocol: on context full, new Debugger instance reads journal to restore debug history
- Pattern matching: maintain internal catalog of common error categories for fast classification
- Confidence reporting: never guess -- always report confidence level for each analysis
- Degradation (P2): journal write failure -> console log + continue; never abort on non-core failures
- Stateless per-request: each DEBUG_REQUEST is independent; cross-request context comes only from journal
